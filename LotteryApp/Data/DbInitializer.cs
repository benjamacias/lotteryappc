...
public static class DbInitializer
{
    public static void EnsureCreatedAndSeed(AppDbContext db)
    {
        ...
        if (!db.Clients.Any())
        {
            ...
            db.Debts.AddRange(
                new Debt { ClientId = c1.Id, Date = DateTime.Today.AddDays(-5), Amount = 5000m, Description = "Jugadas semana 1" },
                new Debt { ClientId = c1.Id, Date = DateTime.Today.AddDays(-2), Amount = 3000m, Description = "Jugadas semana 2" },
                new Debt { ClientId = c2.Id, Date = DateTime.Today.AddDays(-3), Amount = 2000m, Description = "Jugadas" }
            );
